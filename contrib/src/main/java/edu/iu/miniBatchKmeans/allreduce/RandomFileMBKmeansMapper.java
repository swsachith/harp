package edu.iu.miniBatchKmeans.allreduce;
 ...
 */
public class RandomFileMBKmeansMapper extends CollectiveMapper<String, String, Object, Object> {
    ...
    private void runKmeans(List<String> fileNames, Configuration conf, Context context)
        ...
        if (this.isMaster()) {
            loadCentroids(cenTable, dimension, conf.get(MiniBatchKMeansConstants.CFILE), conf);
        }
    ...
    private void calcMSE(Configuration conf) throws IOException {
        ...
        if (this.isMaster()) {
            ...
            Path path = new Path(conf.get(MiniBatchKMeansConstants.WORK_DIR) + "/evaluation");
